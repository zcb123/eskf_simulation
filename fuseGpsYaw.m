function is_fused = fuseGpsYaw(gps_sample_delayed,gps_yaw_offset)
    global states P;
	...
	measured_hdg = wrap_pi(gps_sample_delayed.yaw + gps_yaw_offset);
	...
	predicted_hdg = atan2(ant_vec_ef(1), ant_vec_ef(0));
	...
	SA0 = ant_vec_bf(1);%sinf(ant_yaw);
	SA1 = 2*q1;
	SA2 = SA1*q4;
	SA3 = 2*q2;
	SA4 = SA3*q3;
	SA5 = SA0*(-SA2 + SA4);
	SA6 = ant_vec_bf(0);%cosf(ant_yaw);
	SA7 = 1 - 2*powf(q4, 2);
...
% 	SA11_inv;
	...
	if (~control_status.flags.in_air...
	    && fabsf(yaw_signed_test_ratio_lpf.getState()) > 0.2) 
	...
	is_fused = measurementUpdate(Kfusion, heading_innov_var, heading_innov, hd_deltastates);
