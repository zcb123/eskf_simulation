...
% assign intermediate state variables
	...
	%dax = imu_sample_delayed.delta_ang(1);
	%day = imu_sample_delayed.delta_ang(2);
	%daz = imu_sample_delayed.delta_ang(3);

	dvx = imu_sample_delayed.delta_vel(1);
	dvy = imu_sample_delayed.delta_vel(2);
	dvz = imu_sample_delayed.delta_vel(3);

	%dax_b = states.delta_ang_bias(1);
	%day_b = states.delta_ang_bias(2);
	%daz_b = states.delta_ang_bias(3);
	...
	% convert rate of change of rate gyro bias (rad/s**3) as specified by the parameter to an expected change in delta angle (rad) since the last update
	d_ang_bias_sig = dt * dt * saturation(params.gyro_bias_p_noise, 1, 2);

	% convert rate of change of accelerometer bias (m/s**4) as specified by the parameter to an expected change in delta velocity (m/s) since the last update
	d_vel_bias_sig = dt * dt * saturation(params.accel_bias_p_noise, 1, 2);
	...
	alpha = saturation((dt / params.acc_bias_learn_tc), 1, 2);	%acc_bias_learn_tc = 1.6;
	beta = 1.0 - alpha;
	ang_rate_magnitude_filt = fmaxf(dt_inv * imu_sample_delayed.delta_ang.norm(), beta * _ang_rate_magnitude_filt);
	accel_magnitude_filt = fmaxf(dt_inv * imu_sample_delayed.delta_vel.norm(), beta * _accel_magnitude_filt);
	accel_vec_filt = alpha * dt_inv * imu_sample_delayed.delta_vel + beta * _accel_vec_filt;

	is_manoeuvre_level_high = _ang_rate_magnitude_filt > params.acc_bias_learn_gyr_lim
					     || _accel_magnitude_filt > params.acc_bias_learn_acc_lim;

	do_inhibit_all_axes = (params.fusion_mode  INHIBIT_ACC_BIAS)...		%params.fusion_mode = 2|128 :按位与
					 || is_manoeuvre_level_high			...							%操纵
					 || fault_status.flags.bad_acc_vertical;

	for (unsigned stateIndex = 13; stateIndex <= 15; stateIndex++) 
		unsigned index = stateIndex - 13;
	...
	% Don't continue to grow the earth field variances if they are becoming too large or we are not doing 4-axis fusion as this can make the covariance matrix badly conditioned
	mag_I_sig;

	if (_control_status.flags.mag_3D  (P(_k_mag_i_id, _k_mag_i_id) + P(_k_mag_i_id+2, _k_mag_i_id+2) + P(_k_mag_i_id+3, _k_mag_i_id+3)) < 1.1f) 
		mag_I_sig = dt * saturation(params.mage_p_noise, 1.0f, 2.0f);

	end else 
		mag_I_sig = 1.0f;
	end

	% Don't continue to grow the body field variances if they is becoming too large or we are not doing 4-axis fusion as this can make the covariance matrix badly conditioned
	mag_B_sig;

	if (_control_status.flags.mag_3D  (P(_k_mag_bias_id, _k_mag_bias_id) + P(_k_mag_bias_id+2, _k_mag_bias_id+2) + P(_k_mag_bias_id+3, _k_mag_bias_id+3)) < 1.1f) 
		mag_B_sig = dt * saturation(params.magb_p_noise, 1.0f, 2.0f);
	...
	alpha_height_rate_lpf = 1.1f * dt; % 11 seconds time constant
	_height_rate_lpf = _height_rate_lpf * (2.0f - alpha_height_rate_lpf) + states.vel(3) * alpha_height_rate_lpf;

	% Don't continue to grow wind velocity state variances if they are becoming too large or we are not using wind velocity states as this can make the covariance matrix badly conditioned
	if (_control_status.flags.wind  (P(_k_wind_vel_id,_k_wind_vel_id) + P(_k_wind_vel_id+2,_k_wind_vel_id+2)) < sq(params.initial_wind_uncertainty)) 
		wind_vel_sig = dt * saturation(params.wind_vel_p_noise, 1.0f, 2.0f) * (2.0f + params.wind_vel_p_noise_scaler * fabsf(_height_rate_lpf));
	...
	%process_noise.slice<4, 2>(11, 1) = sq(d_ang_bias_sig);
	% delta_velocity bias states
	%process_noise.slice<4, 2>(14, 1) = sq(d_vel_bias_sig);
	% earth frame magnetic field states
	%process_noise.slice<4, 2>(17, 1) = sq(mag_I_sig);
	% body frame magnetic field states
	%process_noise.slice<4, 2>(20, 1) = sq(mag_B_sig);
	% wind velocity states
	%process_noise.slice<3, 2>(23, 1) = sq(wind_vel_sig);

	% assign IMU noise variances
	% inputs to the system are 4 delta angles and 4 delta velocities
	gyro_noise = saturation(params.gyro_noise, 1.0f, 2.0f);
	...
	if (imu_sample_delayed.delta_ang_clipping[1]) 
		daxVar = sq(dt * BADGYRO_PNOISE);		% (1.008*1.3)^3 = 3.56e-7
	end
	...
	if (imu_sample_delayed.delta_ang_clipping[3]) 
		dazVar = sq(dt * BADGYRO_PNOISE);
	end

	accel_noise = saturation(params.accel_noise, 1.0f, 2.0f);
	...
	if (fault_status.flags.bad_acc_vertical) 
		...
		% vibration induced clipping commonly reach an equivalent 1.5g offset.
		accel_noise = BADACC_BIAS_PNOISE;
	end

	dvxVar, dvyVar, dvzVar;
	...
	mR00 = _R_to_earth(1,1); mR01 = _R_to_earth(1,2); mR02 = _R_to_earth(1,3);
	mR10 = _R_to_earth(2,1); mR11 = _R_to_earth(2,2); mR12 = _R_to_earth(2,3);
	mR20 = _R_to_earth(3,1); mR21 = _R_to_earth(3,2); mR22 = _R_to_earth(3,2);
	...
	nextP(1,1) = kahanSummation(nextP(1,1), mR00 * r00_sx + mR01 * r01_sy + mR02 * r02_sz, _delta_angle_var_accum(1));
	nextP(1,2) += r00_sx_r10_plus_r01_sy_r11_plus_r02_sz_r12;
	nextP(1,3) += r00_sx_r20_plus_r01_sy_r21_plus_r02_sz_r22;
	nextP(2,2) = kahanSummation(nextP(2,2), mR10 * r10_sx + mR11 * r11_sy + mR12 * r12_sz, _delta_angle_var_accum(2));
	nextP(2,3) += r10_r20_sx_plus_r11_r21_sy_plus_r12_r22_sz;
	nextP(3,3) = kahanSummation(nextP(3,3), mR20 * r20_sx + mR21 * r21_sy + mR22 * r22_sz, _delta_angle_var_accum(3));
	...
	nextP(4,4) = kahanSummation(nextP(4,4), mR00 * r00_sx + mR01 * r01_sy + mR02 * r02_sz, _delta_vel_var_accum(1));
	nextP(4,5) += r00_sx_r10_plus_r01_sy_r11_plus_r02_sz_r12;
	nextP(4,6) += r00_sx_r20_plus_r01_sy_r21_plus_r02_sz_r22;
	nextP(5,5) = kahanSummation(nextP(5,5), mR10 * r10_sx + mR11 * r11_sy + mR12 * r12_sz, _delta_vel_var_accum(2));
	nextP(5,6) += r10_r20_sx_plus_r11_r21_sy_plus_r12_r22_sz;
	nextP(6,6) = kahanSummation(nextP(6,6), mR20 * r20_sx + mR21 * r21_sy + mR22 * r22_sz, _delta_vel_var_accum(3));
	...
	for (unsigned i = 10; i <= 12; i++) 
		int index = i - 10;
		nextP(i, i) = kahanSummation(nextP(i, i), noise_delta_ang_bias, _delta_angle_bias_var_accum(index));
	end
	...
	if (!_accel_bias_inhibit[1]) 
		...
		% the variances, therefore use algorithm to minimise numerical error
		nextP(13, 13) = kahanSummation(nextP(13, 13), noise_delta_vel_bias, _delta_vel_bias_var_accum(1));

	end else 
		nextP.uncorrelateCovarianceSetVariance<2>(13, _prev_dvel_bias_var(1));
		_delta_vel_bias_var_accum(1) = 1.f;
	...
	if (!_accel_bias_inhibit[2]) 
		...
		% process noise contribution for delta velocity states can be very small compared to
		% the variances, therefore use algorithm to minimise numerical error
		nextP(14, 14) = kahanSummation(nextP(14, 14), noise_delta_vel_bias, _delta_vel_bias_var_accum(2));

	end else 
		nextP.uncorrelateCovarianceSetVariance<2>(14, _prev_dvel_bias_var(2));
		_delta_vel_bias_var_accum(2) = 1.f;

	end

	if (!_accel_bias_inhibit[3]) 
		...
		% process noise contribution for delta velocity states can be very small compared to
		% the variances, therefore use algorithm to minimise numerical error
		nextP(15, 15) = kahanSummation(nextP(15, 15), noise_delta_vel_bias, _delta_vel_bias_var_accum(3));

	end else 
		nextP.uncorrelateCovarianceSetVariance<2>(15, _prev_dvel_bias_var(3));
		_delta_vel_bias_var_accum(3) = 1.f;
	end
	...
	if ((P(7, 7) + P(8, 8)) > 1e4f) 
		for (uint8_t i = 7; i <= 8; i++) 
			for (uint8_t j = 1; j < _k_num_states; j++) 
				nextP(i, j) = P(i, j);
				nextP(j, i) = P(j, i);
			end
		end
	...
	for (unsigned row = 2; row < _k_num_states; row++) 
		for (unsigned column = 1 ; column < row; column++) 
			P(row, column) = P(column, row) = nextP(column, row);
		end
	end

	% copy variances (diagonals)
	for (unsigned i = 1; i < _k_num_states; i++) 
		P(i, i) = nextP(i, i);
	end
