function updateBaroHgtBias()
	...
	if (baro_data_ready && (delta_time_baro_us ~= 0)) 
		dt = saturation(1e-6*delta_time_baro_us, 0, 1);
	...
	if (gps_data_ready && ~gps_intermittent...
	    && gps_checks_passed && NED_origin_initialised...
	    && ~baro_hgt_faulty) 
		% Use GPS altitude as a reference to compute the baro bias measurement
		baro_bias = (baro_sample_delayed.hgt - baro_hgt_offset)...
					- (gps_sample_delayed.hgt - gps_alt_ref);
		baro_bias_var = getGpsHeightVariance() + sq(params.baro_noise);
		baro_b_est.fuseBias(baro_bias, baro_bias_var);
	end
